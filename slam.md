...
**前端（Odometry 部分）：**
    ...
    角点特征：前端对当前帧中每个角点特征，找出前一帧中的两个最近点，以此形成一条线。当前帧的角点特征与前一帧的这条线进行点-线匹配。
    面点特征：对于当前帧的每个面点特征，前端找到前一帧中的三个最近点，构成一个平面，然后将当前面点特征与这个平面进行点-面匹配。
    优化：前端将所有点-线和点-面残差放入 Ceres 优化，优化位姿变换。因为是对所有特征点进行操作，计算量较大。

**后端（Mapping 部分）：**

    下采样：后端会先对当前帧的角点和面点特征进行下采样，以减少计算量。下采样后的特征点才会用于后续优化。
    局部地图特征（PCA）：后端不是简单地直接与前一帧的点做匹配，而是从局部地图中（通常是累积的多帧）寻找最近的5个邻域点。
        角点特征：后端通过 PCA 分析这些邻域点，判断它们是否呈现出线性特征。如果 PCA 特征表明是线性结构，那么就与当前帧角点做点-线残差构建。
        面点特征：对于面点特征，后端同样从邻域点中构造一个平面，然后将当前面点特征与该平面进行点-面残差计算。
    优化：后端将这些残差（点-线和点-面）传递给 Ceres 进行优化，优化整个局部地图的位姿。这是基于局部地图而不是单一前一帧的匹配，因此具有更强的全局一致性。
